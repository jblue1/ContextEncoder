...
def calc_losses(image_contexts, images, use_gpu, generator, discriminator):
    ...
    disc_loss = discriminator_loss(real_output, fake_output)
    gen_loss = generator_loss(fake_output, images, generated_images)
    l2_gen_loss = generator_loss(fake_output, images, generated_images, use_adv=False)
...
def save_pictures(broken_image_batch, image_batch, epoch, use_gpu, save_dir, generator, prefix='',
                  num_pictures=5):
    """
    ...
    gen_image_batch = generator(broken_image_batch, training=False)
    if len(image_batch) < num_pictures:
        num_pictures = 1

    if use_gpu:
        broken_image_batch = tf.transpose(broken_image_batch, (0, 2, 3, 1))
        image_batch = tf.transpose(image_batch, (0, 2, 3, 1))
        gen_image_batch = tf.transpose(gen_image_batch, (0, 2, 3, 1))
    ...
    for i in range(num_pictures):
        ...
        axs[0].imshow(image_batch[i, :, :, 0], cmap='Greys')
        axs[0].set_title('Image')
        axs[1].imshow(broken_image_batch[i, :, :, 0], cmap='Greys')
        axs[1].set_title('Broken Image')
        axs[2].imshow(gen_image_batch[i, :, :, 0], cmap='Greys')
...
@click.option('--num_events', default=38265)
...
def main(data_path, indices_path, num_events, batch_size, use_gpu, epochs, lr, run_number):
    print('Entered main()')
    if not use_gpu:
    ...
    write_info_file(save_dir, data_path, batch_size, use_gpu, epochs, lr, run_number)

    train_dataset, val_dataset = load_data.load_data(data_path, indices_path, num_events)
